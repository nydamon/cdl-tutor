...
// State data (bundled)
const states = {
  NJ: {state:'NJ',state_name:'New Jersey',topics:[{name:'General Knowledge',must_memorize:['4-second following distance','5-6 seconds for trucks']},{name:'Air Brakes',must_memorize:['60 psi warning','20-45 psi spring brakes']},{name:'Hazardous Materials',must_memorize:['0.04% BAC']}]},
  TX: {state:'TX',state_name:'Texas',topics:[{name:'General Knowledge',must_memorize:['4-second following distance','55 mph max']},{name:'Air Brakes',must_memorize:['60 psi warning','20-45 psi spring brakes']},{name:'Hazardous Materials',must_memorize:['0.04% BAC']}]},
  ...
  NY: {state:'NY',state_name:'New York',topics:[{name:'General Knowledge',must_memorize:['4-second following distance']},{name:'Air Brakes',must_memorize:['60 psi warning']},{name:'Hazardous Materials',must_memorize:['0.04% BAC','TWIC card']}]}
};
...
const server = createServer(async (req, res) => {
  ...
  const routes = {
    ...
    '/api/states': {json:statesList},
    '/api/state/NJ': {json:states.NJ},
    '/api/state/TX': {json:states.TX},
    '/api/state/CA': {json:states.CA},
    '/api/state/FL': {json:states.FL},
    '/api/state/NY': {json:states.NY}
  };
  ...
  // Dynamic state route
  if (!route && pathname.startsWith('/api/state/')) {
    const code = pathname.split('/').pop().toUpperCase();
    route = {json: states[code] || {state:code,state_name:code,message:'Coming soon'}};
  }
...
server.listen(PORT, () => console.log(`CDL Tutor on ${PORT}`));
